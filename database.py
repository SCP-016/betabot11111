...
class Database:
    def __init__(self):
        ...
        # Run init synchronously at startup
        asyncio.get_event_loop().run_until_complete(self._init_tables())
    ...
    def get_medias(self, folder_id: int) -> list[dict]:
        async def _():
            async with self._make_client() as c:
                ...
                return [
                    {"id": r[0], "file_id": r[1], "media_type": r[2],
    ...
    def get_media(self, media_id: int) -> dict | None:
        async def _():
            async with self._make_client() as c:
                ...
                if rs.rows:
                    r = rs.rows[0]
                    return {"id": r[0], "file_id": r[1], "media_type": r[2], "caption": r[3]}
                return None
        return self._run(_())
